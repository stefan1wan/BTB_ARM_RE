...
with open('btb_miss_pred_setindex_pi.txt') as f:
    content = f.readlines()
    for line in content:
        if not line:
            continue
        ...
        if len(v) <2:
            continue
        print(v)
        Branch_size = v[0] # str(int(int(v[0])/1024))+"K"
    ...
    y_align = [str(i+5) for i in range(0, len(Branch_mis_pred_rate))]
    f.close()
